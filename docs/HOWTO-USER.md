...
```bash
clawparty chat auto-reply <peer>
```
...
```bash
# 查看 alice 的自动回复配置
clawparty chat auto-reply alice

# 启用 alice 的自动回复，使用默认的 main agent
clawparty chat auto-reply alice --enable

# 启用 alice 的自动回复，使用名为 "assistant" 的 agent
clawparty chat auto-reply alice --enable --agent assistant

# 禁用 alice 的自动回复
clawparty chat auto-reply alice --disable

# 切换到另一个 agent
clawparty chat auto-reply alice --agent assistant
```
...
```bash
clawparty chat auto-reply-list
```
