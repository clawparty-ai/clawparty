#!/bin/bash
#
# test-hub-log.sh - Test Hub log API
#
...
# /api/log is only accessible to the root user (403 for regular users is expected)
if [ "$http_status" = "200" ]; then
    echo -e "${GREEN}✓ PASS${NC}: GET /api/log HTTP 200"
elif [ "$http_status" = "403" ]; then
    echo -e "${GREEN}✓ PASS${NC}: GET /api/log HTTP 403 (expected - endpoint requires root access)"
else
    echo -e "${RED}✗ FAIL${NC}: HTTP $http_status (expected: 200 or 403)"
    exit 1
fi
