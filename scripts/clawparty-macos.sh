...
#       2. 配置文件      0#Agent config.toml 是否存在、api_key/provider/model 是否配置
#                         ClawParty config.toml 是否存在
#       3. 端口冲突      7778 端口是否被占用
...
run_checks() {
    ...
    local zero_config="$CLAWPARTY_HOME/.zeroclaw/config.toml"
    printf "  %-40s " "0#Agent config"
    if [ -f "$zero_config" ]; then
        echo -e "${GREEN}✓${NC} $zero_config"
        passed_count=$((passed_count + 1))
        # Check key fields
        for field in "api_key" "default_provider" "default_model"; do
            printf "    %-36s " "$field"
            local value
            value=$(grep -E "^[[:space:]]*${field}[[:space:]]*=" "$zero_config" 2>/dev/null | tail -1 | sed 's/.*=[[:space:]]*"//;s/".*//' || true)
            if [ -n "$value" ] && [ "$value" != '""' ]; then
                echo -e "${GREEN}✓${NC} ${value:0:40}"
            else
                echo -e "${YELLOW}⚠${NC}  not set or empty"
                problem_count=$((problem_count + 1))
            fi
        done
    else
...
run_fixes() {
    ...
    # 0#Agent config
    local zero_dir="$CLAWPARTY_HOME/.zeroclaw"
    local zero_config="$zero_dir/config.toml"
    if [ ! -f "$zero_config" ]; then
        mkdir -p "$zero_dir"
        cat > "$zero_config" << 'TOML'
# 0#Agent Configuration
# Generated by ClawParty check-fix
