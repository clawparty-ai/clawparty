...
async fn main() -> anyhow::Result<()> {
    ...
    // Set engine for sync_agents_from_fs (must be before sync so agents get correct engine type)
    crate::proxy::set_engine(&args.engine);
...
async fn run_service_mode(args: Args, first_run_api_key: Option<String>, _first_run_provider: String, _first_run_model: String) -> anyhow::Result<(Option<AgentManager>, Option<ZeroClawDaemon>)> {
    ...
    // Auto-sync all agents' engine to match CLI flag (fixes stale records from previous engine mode)
    if let Ok(conn) = rusqlite::Connection::open(format!("{}/clawparty.db", data_dir)) {
        for agent_cfg in &agent_configs {
            if agent_cfg.engine != engine {
                ts_print!("Syncing {} engine: {} -> {}", agent_cfg.agent_name, agent_cfg.engine, engine);
                let _ = conn.execute("UPDATE agents SET engine=?1 WHERE agent_name=?2", rusqlite::params![engine, agent_cfg.agent_name]);
            }
        }
    }
