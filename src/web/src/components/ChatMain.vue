<template>
  <main ref="chatMainEl" class="chat-main">
    <ChatHeader
      ...
      @toggleAIPanel="togglePanel('aipanel')"
      ...
      >
        ...
        <div class="message-body">
          <div v-if="msg.isTyping && !chat.isGroupChat" class="typing-indicator">
            ...
            <span class="typing-dot"></span>
            <span class="typing-dot"></span>
            <button class="stop-thinking-btn" @click="$emit('stop-agent')" title="停止思考 (Esc)">⏹ 停止</button>
          </div>
          <template v-else>
      ...
      文件上传成功，保存在：{{ file.path }}
                ...
                <div v-if="msg.thinking && !msg.isSystemHint" class="thinking-section">
                  <div class="thinking-header" @click="msg._thinkingExpanded = !msg._thinkingExpanded">
                    ...
                    <span class="thinking-toggle">{{ msg._thinkingExpanded ? '▾' : '▸' }}</span>
                  </div>
                  <div v-if="msg._thinkingExpanded" class="thinking-content">{{ msg.thinking }}</div>
                </div>
                <!-- Quote preview -->
...
const emit = defineEmits(['send', 'update:modelValue', 'switchSession', 'deleteGroup', 'leaveGroup', 'back', 'send-images', 'send-files', 'clear-quote', 'stop-agent'])
...
const thinkingLog = computed(() => {
  const msgs = props.chat.messages || []
  const entries = []
  for (const msg of msgs) {
    if (msg.thinking) {
      const trimmed = msg.thinking.trim()
      if (trimmed) {
        entries.push({
          time: msg.time || '',
          content: trimmed,
        ...
        if (data.type === 'chunk') {
          ...
          try {
            ...
            try {
              ...
              addRefreshLog('warn', 'Initial JSON parse failed, attempting repair...')
              const repaired = repairJson(jsonText)
              result = JSON.parse(repaired)
              addRefreshLog('info', 'JSON repaired successfully')
            }
            const changes = []
            ...
            addRefreshLog('error', 'Raw response (first 500 chars): ' + fullResponse.substring(0, 500))
          }
          persistLastAnalyzed(newMessages[newMessages.length - 1].timestamp)
...
.message.sent .thinking-section {
  border-top-color: rgba(255,255,255,0.2);
}

.message.sent .thinking-header {
  color: rgba(255,255,255,0.7);
}

.message.sent .thinking-header:hover {
