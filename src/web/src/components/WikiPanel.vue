  ...
  <div class="wiki-panel" :class="{ fullscreen: isFullscreen }" :style="{ height: isFullscreen ? '100vh' : panelHeight + 'px' }">
    ...
    <div v-show="expanded" class="wiki-panel-body">
      ...
      <div v-else-if="activeTab === 'graph'" class="wiki-graph-wrapper">
        <div class="wiki-graph-layout">
          <div class="wiki-graph-node-list">
            <input
              v-model="nodeSearch"
              type="text"
              placeholder="搜索节点..."
              class="wiki-graph-filter-input"
            />
            <div class="wiki-graph-node-items">
              <label
                v-for="node in filteredNodeList"
                :key="node.id"
                class="wiki-graph-filter-item"
                :class="{ checked: selectedNodes.has(String(node.id)) }"
                @click="toggleNode(String(node.id))"
              >
                <span class="filter-dot" :style="{ background: node.color }"></span>
                <span class="filter-name">{{ node.name }}</span>
                <input type="checkbox" :checked="selectedNodes.has(String(node.id))" />
              </label>
            </div>
          </div>
...
// Node selection for graph filtering
const allNodes = ref([])
const allLinks = ref([])
...
const filteredNodeList = computed(() => {
  ...
  return allNodes.value.filter(n => n.name.toLowerCase().includes(q))
})
...
const rebuildGraph = () => {
  ...
  if (sel.size === 0) {
    // No selection: show all edges, clear node borders
    cyInstance.elements().style('display', 'element')
    cyInstance.nodes().forEach(n => n.style('border-width', '0'))
    return
  }
  // Always show all nodes, highlight selected ones
  cyInstance.nodes().forEach(n => {
    const isSel = sel.has(n.id())
    n.style('display', 'element')
    n.style('border-width', isSel ? '3' : '0')
    n.style('border-color', '#ff6b35')
    n.style('opacity', sel.size > 0 && !isSel ? '0.4' : '1')
  })
  // Only show edges between two selected nodes
  cyInstance.edges().forEach(e => {
    const visible = sel.has(e.source().id()) && sel.has(e.target().id())
...
const renderGraph = async () => {
  ...
  try {
    ...
    allNodes.value = nodes.map(n => ({
      id: String(n.id),
      name: n.name,
      category: n.category,
      color: colors[n.category] || colors.page
    }))
    ...
    // Tap node to toggle selection - highlight and rebuild
    cyInstance.on('tap', 'node', (evt) => {
      const id = evt.target.id()
      toggleNode(id)
    })

    // Apply existing selection filter
    if (selectedNodes.value.size >= 2) {
      rebuildGraph()
    }
...
.wiki-graph-node-items {
  flex: 1;
  overflow-y: auto;
}

.wiki-graph-filter-item {
  display: flex;
  align-items: center;
  gap: 6px;
  padding: 4px 8px;
...
.wiki-graph-filter-item:hover {
  background: #e8f0fe;
}

.wiki-graph-filter-item.checked {
  background: #e3f2fd;
}

.wiki-graph-filter-item input[type="checkbox"] {
  margin: 0 0 0 auto;
}
