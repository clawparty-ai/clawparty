  ...
  var routes = Object.entries({
    ...
    '/api/join-party': {
      'POST': function (_, req) {
        ...
        function resolveRegUrl(url) {
          ...
          var isPort = true
          for (var i = 0; i < url.length && isPort; i++) {
            var ch = url.charAt(i)
            if (ch < '0' || ch > '9') { isPort = false }
          }
    ...
    '/api/wiki/{agent}/init': {
      'POST': function ({ agent }) {
        ...
        var row = db.getAgent(agent)
        if (!row) return response(404, { error: 'Agent not found' })
        var wikiDir = os.path.join(row.workspace_dir, 'wiki')
        var rawDir = os.path.join(wikiDir, 'raw')
        var entitiesDir = os.path.join(wikiDir, 'entities')
        var conceptsDir = os.path.join(wikiDir, 'concepts')
        var pagesDir = os.path.join(wikiDir, 'pages')

        try {
          os.mkdir(wikiDir, { recursive: true })
          os.mkdir(rawDir, { recursive: true })
          os.mkdir(entitiesDir, { recursive: true })
          os.mkdir(conceptsDir, { recursive: true })
          os.mkdir(pagesDir, { recursive: true })

          var indexPath = os.path.join(wikiDir, 'index.md')
          try { os.read(indexPath) } catch {
            os.write(indexPath, '# Wiki 目录\n\nWiki 初始化于 ' + new Date().toISOString() + '\n')
          }

          var logPath = os.path.join(wikiDir, 'log.md')
          try { os.read(logPath) } catch {
            os.write(logPath, '# Wiki 日志\n\n')
          }

          var schemaPath = os.path.join(wikiDir, 'schema.md')
          try { os.read(schemaPath) } catch {
            var schemaContent = '# Wiki 维护规范\n\n## 目录结构\n\n- `raw/` — 原始资料（不可变）\n- `entities/` — 实体页面（人物、地点、事物）\n- `concepts/` — 概念页面（抽象主题）\n- `pages/` — 通用页面\n\n## 链接规范\n\n使用 `[[Page Name]]` 创建内部链接。\n\n## 维护要求\n\n1. 每次处理新资料后更新 `index.md`\n2. 在 `log.md` 中记录变更\n3. 保持交叉引用一致\n'
            os.write(schemaPath, schemaContent)
          }

          return response(200, { message: 'Wiki initialized', path: wikiDir })
        } catch (e) {
          return response(500, { error: 'Failed to init wiki: ' + (e?.message || e) })
        }
      }
    },
    ...
    '/api/wiki/{agent}/tree': {
      'GET': function ({ agent }, req) {
        ...
        var url = new URL(req.head.path, 'http://localhost')
        var subPath = url.searchParams.get('path') || ''
        if (subPath.indexOf('..') >= 0) return response(403, { error: 'Forbidden path' })

        var wikiDir = os.path.join(row.workspace_dir, 'wiki')
        var targetDir = subPath ? os.path.join(wikiDir, subPath) : wikiDir

        try {
          var st = os.stat(targetDir)
          if (!st || !st.isDirectory()) {
            return response(200, { agent: agent, path: subPath, files: [] })
          }
        } catch (e) {
          return response(200, { agent: agent, path: subPath, files: [] })
        }
        ...
        var entries = []
        try { entries = os.readDir(targetDir) } catch (e) { entries = [] }
        ...
        for (var i = 0; i < entries.length; i++) {
          ...
          if (name.endsWith('/')) {
            ...
            var filePath = os.path.join(targetDir, name)
            try {
              var stat = os.stat(filePath)
              if (stat.isFile()) {
                files.push({
    ...
    '/api/wiki/{agent}/file/{name}': {
      'GET': function ({ agent, name }, req) {
        agent = URL.decodeComponent(agent)
        name = URL.decodeComponent(name)
        var row = db.getAgent(agent)
        if (!row) return response(404, { error: 'Agent not found' })
        ...
        var filePath
        if (subPath) {
          filePath = os.path.join(wikiDir, subPath, name)
        } else {
          filePath = os.path.join(wikiDir, name)
        }
    ...
    '/api/wiki/{agent}/search': {
      'GET': function ({ agent }, req) {
        ...
        function searchDir(dir, relPath) {
          var entries = []
          try { entries = os.readDir(dir) } catch (e) { return }
          for (var i = 0; i < entries.length; i++) {
            var name = entries[i]
            if (name.endsWith('/')) {
              var subDir = name.substring(0, name.length - 1)
              var newRel = relPath ? relPath + '/' + subDir : subDir
              searchDir(os.path.join(dir, subDir), newRel)
            } else if (name.endsWith('.md')) {
              var filePath = os.path.join(dir, name)
              var content = ''
              try { content = os.read(filePath).toString() } catch (e) {}
              var fileRel = relPath ? relPath + '/' + name : name
              var title = name.replace('.md', '')
              var lines = content.split('\n')
              if (lines.length > 0 && lines[0].startsWith('# ')) {
                title = lines[0].substring(2).trim()
              }
              if (title.toLowerCase().indexOf(query) >= 0 || content.toLowerCase().indexOf(query) >= 0) {
                var preview = ''
                for (var j = 0; j < lines.length && !preview; j++) {
                  if (lines[j].toLowerCase().indexOf(query) >= 0) {
                    preview = lines[j].trim().substring(0, 150)
                  }
                }
                results.push({
                  name: name,
    ...
    '/api/wiki/{agent}/graph': {
      'GET': function ({ agent }) {
        ...
        function getNodeId(name, category) {
          ...
          nodes.push({ id: nodeIndex, name: name, category: category })
          return nodeIndex++
        }

        function scanDir(dir, category, relPath) {
          var entries = []
          try { entries = os.readDir(dir) } catch (e) { return }
          for (var i = 0; i < entries.length; i++) {
            var name = entries[i]
            if (!name.endsWith('/') && name.endsWith('.md')) {
              var pageName = name.replace('.md', '')
              var filePath = os.path.join(dir, name)
              var content = ''
              try { content = os.read(filePath).toString() } catch (e) {}
              if (content) {
        ...
        scanDir(os.path.join(wikiDir, 'raw'), 'raw', 'raw')
        scanDir(os.path.join(wikiDir, 'entities'), 'entity', 'entities')
        scanDir(os.path.join(wikiDir, 'concepts'), 'concept', 'concepts')
        scanDir(os.path.join(wikiDir, 'pages'), 'page', 'pages')
        scanDir(wikiDir, 'page', '')

        return response(200, { nodes: nodes, links: links })
      }
    },

    '/api/wiki/{agent}/refresh': {
      'POST': function ({ agent }) {
        agent = URL.decodeComponent(agent)
        var row = db.getAgent(agent)
        if (!row) return response(404, { error: 'Agent not found' })
        return response(200, { message: 'Wiki refreshed', agent: agent })
      }
    },
